    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 p-6">
      ...
      {loading ? (
        ...
        <>
          ...
          <section className="mb-10">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full border bg-gray-800 shadow-sm rounded-xl">
                ...
                <tbody>
                  {events.map((ev) => (
                    <tr key={ev.id} className="border-t hover:bg-gray-800">
                      <td className="p-3">{ev.title}</td>
                      <td className="p-3">{ev.category}</td>
                      <td className="p-3">{ev.venue}</td>
                      <td className="p-3">{new Date(ev.startDate).toLocaleDateString()} - {new Date(ev.endDate).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* Users Section */}
          <section>
            <h2 className="text-2xl font-semibold mb-4 text-gray-100">Registered Users</h2>
            <div className="overflow-x-auto">
              <table className="min-w-full border bg-gray-800 shadow-sm rounded-xl">
                <thead className="bg-gray-700 text-gray-100">
                  <tr>
                ...
                <tbody>
                  {users.map((u) => (
                    <tr key={u.id} className="border-t hover:bg-gray-800">
